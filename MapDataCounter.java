...
public class MapDataCounter<E extends Comparable<E>> implements DataCounter<E>{

    //OVERVIEW: MapDataCounter implementa una struttura ordinatadi elementi E associando a loro un valore Integer

    //AF: { <key_i,ogg.get(key_i)> | ogg.containsKey(key_i)==T && 0<=i<ogg.size()=n && key_i != null }
    //IR: { ForAll i,j . 0<=key_i<key_j<ogg.size() => key_i!=key_j }

    TreeMap<E,Integer> ogg;

    public MapDataCounter(){
        ogg = new TreeMap<>();
    }
    public MapDataCounter(List<E> ist){
        ogg = new TreeMap<>();
        Integer n;
    ...
    public Iterator<Map.Entry<E,Integer>> getIterator(){
        ...
        //Ordina secondo il Comparator sopra definito
